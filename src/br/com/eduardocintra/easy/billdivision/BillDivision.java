...
public class BillDivision {
    public static void main(String[] args) throws IOException {
        ...
        List<Integer> firstLine = Arrays.stream(sc.nextLine().split("\\s")).map(Integer::parseInt).collect(toList());
        int size = firstLine.get(0);
        int indexAnaDoesntEat = firstLine.get(1);
        List<Integer> items = Arrays.stream(sc.nextLine().split("\\s")).map(Integer::parseInt).collect(toList());
        if(size != items.size()) {
